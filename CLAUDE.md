...
## D20 - ElastiCache had no authentication at all

`modules/elasticache-redis`, `envs/dev/d20-redis-auth-token.tf`, and the token's
SSM parameter in `modules/ssm-app-parameters`.

### The live value, not the intention

`redis_auth_token = ""` in `terraform.tfvars`, and
`describe-replication-groups` agrees: `AuthTokenEnabled: false`,
`TransitEncryptionEnabled: false` on `kambriq-dev-redis` (redis **7.1.0**,
cluster mode disabled, at-rest encryption on). No password. Anyone who reaches
the VPC reaches the session store and the five queues - KBS, CORE, KAMNET,
NOTIFICATIONS, DUNNING - and since D1 that is the VPC production will share,
which is why this is an ISO defect and not a dev nicety.

### It is heavier than one flag, and the engine version decides how heavy

AUTH cannot be enabled alone: *"AUTH can only be enabled for encryption
in-transit enabled clusters"*. And enabling in-transit encryption on an existing
cluster is only supported *"on replication groups running Valkey 7.2 and later,
and Redis OSS version 7 and later"* - **7.1.0 qualifies, so the cluster is
modified in place and nothing is lost.** Below 7.0.5 the provider would have
forced a replacement, which would have dropped every session and every queued
job; that sentence is here so the next environment is checked before the change
rather than after.

### Four steps, in this order, and the order is the safety

1. **Application first** (`feat/d20-redis-tls-client`): one helper,
   `redisConnectionOptions`, used by `RedisService`, the BullMQ root connection
   and `prisma/bootstrap-admins.ts` - the one-off ECS task that enqueues the
   administrator mail on every deploy, and the client that gets forgotten
   because it is not the API. Both options default to off, so the same build
   works against today's cluster and tomorrow's.
2. **Apply one**: `transit_encryption_mode = "preferred"` and
   `auth_token_update_strategy = "ROTATE"`. Both encrypted and unencrypted, both
   authenticated and unauthenticated, are accepted. Nothing breaks.
3. **Deploy the API with `REDIS_TLS=true` and the `REDIS_PASSWORD` secret**, and
   watch a queued job be processed end to end - a session store that answers
   PING is not a queue that still works.
4. **Apply two**: `"required"` and `"SET"`. This is the apply that drops
   unencrypted connections and makes the token mandatory.

### The token

Generated by `random_password`, 48 characters, `override_special = "!&#$^<>-"` -
AWS restricts non-alphanumerics to exactly those eight and requires 16 to 128
printable characters, so a general-purpose generated password is refused at
modify time. It lives in the state (like the RDS master password since A30) and
in SSM as a SecureString at `/kambriq/{env}/api/REDIS_PASSWORD`, reaching the
task through the ECS `secrets:` block - **not** the runtime SSM reader G3 chose
for payment channels, because this one is needed to open the connection at boot.
Named `REDIS_PASSWORD` to match what the application reads.

### Cost, and what it does not cover

**Nothing in AWS**: AUTH and in-transit encryption are free on ElastiCache. The
cost is the application change and the risk window between the two applies.

It does not cover: rotation - ElastiCache AUTH has no managed rotation, and AWS
now points at RBAC with Secrets Manager for that; the 5 % to 10 % latency TLS
adds; and anyone inside the VPC who has the token. A password is not network
isolation, and the security group remains the thing that decides who may reach
port 6379 at all.
