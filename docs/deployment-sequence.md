...
The DB master password needs no input: it is generated by `random_password` in
`envs/dev/a30-db-password.tf` and thereafter ignored by `modules/rds-postgres`.
There is no `db_password` variable to set. (A30.)
