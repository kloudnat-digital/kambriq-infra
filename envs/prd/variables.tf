...
# --- prod-only knobs (dev leaves these at their safe defaults) --------------
variable "rds_multi_az" {
  description = "Multi-AZ failover for the database. Lean prod: false."
  type        = bool
  default     = false
}

variable "rds_deletion_protection" {
  description = "Refuse deletion of the prod database at the AWS API level."
  type        = bool
  default     = true
}

variable "prod_public_subnet_cidrs" {
  description = "Prod's own public subnets inside the shared VPC (ALB + NAT)."
  type        = list(string)
  default     = ["10.0.4.0/24", "10.0.5.0/24"]
}

variable "prod_private_subnet_cidrs" {
  description = "Prod's own private subnets inside the shared VPC (tasks, RDS, Redis)."
  type        = list(string)
  default     = ["10.0.6.0/24", "10.0.7.0/24"]
}

variable "prod_azs" {
  description = "AZs for prod subnets, matching the shared VPC's existing pair."
  type        = list(string)
