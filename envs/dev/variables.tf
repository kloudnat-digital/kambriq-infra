...
# D20. `redis_auth_token` was a variable here, set to "" in terraform.tfvars -
# which is to say the live cluster had no password at all. It is gone rather
# than filled in: the token is generated by `random_password.redis_auth_token`
# (d20-redis-auth-token.tf), so there is no value to keep out of the repository
# and none to pass on a command line the apply never sees.

variable "redis_transit_encryption_enabled" {
  description = "Enable Redis in-transit encryption. Required before AUTH: AWS refuses a token without it."
  type        = bool
  default     = false
}

# D20. Two steps, because AWS makes it two steps on an existing cluster:
# `preferred` accepts encrypted and unencrypted connections while the
# application is deployed with TLS, `required` then drops the unencrypted ones.
variable "redis_transit_encryption_mode" {
  description = "preferred (both accepted) or required (TLS only). preferred first on an existing cluster."
  type        = string
  default     = null
}

# D20. ROTATE makes the token optional (the cluster also still accepts
# unauthenticated connections), SET makes it required. ROTATE for the first
# apply, SET for the one that closes the door.
variable "redis_auth_token_update_strategy" {
  description = "ROTATE (token optional) or SET (token required)."
  type        = string
  default     = "ROTATE"
}
